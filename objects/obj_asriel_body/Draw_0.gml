...
if(global.debug == 1) {
    if(keyboard_check_pressed(83) && aligncon == 3) {
        type= 0;
        starcon= 1;
    }
    if(keyboard_check_pressed(66) && aligncon == 3) {
        type= 1;
        starcon= 1;
    }
    if(keyboard_check_pressed(87) && aligncon == 3) {
        type= 1;
        bladecon= 1;
    }
    if(keyboard_check_pressed(71) && aligncon == 3) {
        type= 1;
        guncon= 1;
    }
    if(keyboard_check_pressed(72) && aligncon == 3) {
        type= 1;
        gonercon= 1;
    }
}
if(starcon > 0) {
    ...
    if(starcon == 12) {
        if(type == 0) gen= instance_create(0, 0, obj_stormstar_gen );
        if(type == 1) gen= instance_create(0, 0, obj_rainbowbolt_realgen );
        gen.h_mode= h_mode;
        starcon= 13;
        alarm[5]= 300;
...
if(specialnormal == 0 && global.mnfight == 0 && global.myfight == 0 && (global.bmenuno == 1 || global.bmenuno == 2)) {
    ...
    while(i < len) {
        ...
        draw_text_transformed(110 + i * 16 + sin((siner + i) / 5) * 8, 270 + cos((siner + i) / 5) * 4, string_hash_to_newline(letter), 1, 1, 0);
        i++;
    }
}
