...
if(global.mnfight == 2) {
    if(attacked == 0) {
        ...
        if(mycommand >= 0)
            global.msg[0]= "* Astigmatism gazes through&  your soul.";
        if(mycommand >= 30)
            global.msg[0]= "* Astigmatism gives you a&  hypnotizing glare.";
        if(mycommand >= 70)
            global.msg[0]= "* Astigmatism clicks its teeth.";
        if(mycommand >= 90) global.msg[0]= "* Smells like glasses.";
        if(mercymod < -100)
            global.msg[0]= "* Astigmatism has gone&  bloodshot.";
        if(mercymod > 100)
            global.msg[0]= "* Astigmatism doesn\\'t care&  about fighting anymore.";
        if(global.monsterhp[myself] < global.monstermaxhp[myself] / 3)
            global.msg[0]= "* Astigmatism is watering.";
        attacked= 1;
    }
    if(mercymod == 999999) {
        global.turntimer= -1;
...
if(global.myfight == 2 && whatiheard != -1 && global.heard == 0) {
    if(whatiheard == 0) {
        global.msc= 0;
        global.msg[0]= "* ASTIGMATISM - ATK 32 DEF 26&* This relentless bully ALWAYS&  gets its way./^";
        OBJ_WRITER.halt= 3;
    ...
    if(whatiheard == 3) {
        global.msc= 0;
        global.msg[0]= "* You pick on Astigmatism./^";
        OBJ_WRITER.halt= 3;
        if(pickon == 0) mercymod= -120;
        if(pickon == 1) mercymod= 120;
        iii= instance_create(global.idealborder[0], global.idealborder[2], OBJ_WRITER );
        with(iii) halt= 0;
    }
    if(whatiheard == 1) {
        global.msc= 0;
        global.msg[0]= "* You don\\'t pick on Astigmatism./^";
        if(pickon == 1) mercymod= -120;
        if(pickon == 0) mercymod= 120;
        OBJ_WRITER.halt= 3;
        iii= instance_create(global.idealborder[0], global.idealborder[2], OBJ_WRITER );
    ...
    if(whatiheard == 4) {
        ...
        sum= scr_monstersum();
        if(sum == 1) global.msg[0]= "* You stand up to Astigmatism./^";
        if(sum > 1)
            global.msg[0]= "* You stand up to Astigmatism^1.&* The other monsters are&  impressed./^";
