...
if(global.mnfight == 2) {
    ...
    if(attacked == 0) {
        ...
        pop= scr_monstersum();
        if(pop == 1) {
            if(mycommand >= 0) global.msg[0]= "* 03 is trembling.";
            if(mycommand >= 25)
                global.msg[0]= "* 03 is holding her face in&  her hands.";
            if(mycommand >= 50) global.msg[0]= "* 03 stands still.";
            if(mycommand >= 75) global.msg[0]= "* 03 seems sick.";
        }
...
if(global.myfight == 2 && whatiheard != -1 && global.heard == 0) {
    ...
    if(whatiheard == 3) {
        global.msc= 0;
        global.msg[0]= "* You tell RG 03 that she&  should stand up for her/^";
        OBJ_WRITER.halt= 3;
        iii= instance_create(global.idealborder[0], global.idealborder[2], OBJ_WRITER );
        with(iii) halt= 0;
    }
