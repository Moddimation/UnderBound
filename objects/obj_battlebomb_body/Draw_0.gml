...
if(flash == 1) {
    flasher++;
    if(flasher == 1) {
        if(defuse == 0) snd_play(snd_break1 );
        if(defuse == 1) {
            snd_play(snd_break2 );
...
draw_sprite_ext(spr_defusebox , 0, 320, 30, 2, 2, 0, blendb, 1);
blenda= 16777215;
blendb= 32768;
if(type == 0) {
...
if(defuse == 1) {
    if(sprite_index == 492) sprite_index= spr_battlebomb_defuse ;
    if(sprite_index == 1519) sprite_index= spr_tobdogl ;
}
