...
if(onoff > 3) onoff= 0;
draw_set_color(0);
if(!instance_exists(obj_mouthbeam )) {
    if(mode == 0)
...
if(con == 4) {
    cntr++;
    draw_sprite_ext(spr_mouthflash , 0, 274, 202 - sin(siner / 4) * 3, 1, 1, 0, 16777215, (cntr - onoff * 5) / 15);
}
if(con == 5) {
    laugh= 1;
    mode= 2;
...
if(global.debug == 1 && keyboard_check_pressed(16)) {
    if(mode == 0) mode= 1;
    else  mode= 0;
}
if(global.debug == 1 && keyboard_check_pressed(17)) {
    if(laugh == 0) laugh= 1;
    else  laugh= 0;
}
if(laugh == 1) {
