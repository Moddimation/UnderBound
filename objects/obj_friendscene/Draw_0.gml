...
if(lastwhite == 1) {
    __view_set( e__VW.XView, 0, 0 + random(2 + wht * 2) - random(2 - wht * 2) );
    __view_set( e__VW.YView, 0, 0 + random(2 + wht * 2) - random(2 - wht * 2) );
    wht+= 0.003;
    draw_set_alpha(wht);
    draw_set_color(16777215);
    draw_rectangle(-10, -10, 999, 999, 0);
...
if(leg > 0) {
    if(leg == 1) {
        if(legx > room_width + 50) legx-= 10;
        leg_r++;
    }
    if(leg == 2) {
        leg_r++;
        if(legx < room_width + 200) legx+= 10;
        else  leg= 0;
    }
    draw_sprite_ext(spr_mettleg2_flip , 0, legx, 180, 2, 2, sin(leg_r / 4) * 6, 16777215, 1);
}
