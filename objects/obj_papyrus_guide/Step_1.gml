...
if(instance_exists(obj_kitchenchecker ) && spot > 0 && obj_kitchenchecker.level == 0 && global.interact == 0) {
    if(spot == 2) {
        myscore++;
        if(myscore > 3) {
            snd_play(snd_bell );
...
if(spot == 1) {
    ...
    if(x > 183) {
        ...
        solid= 1;
        if(myscore > 10) snd_play(snd_levelup );
        myscore= 0;
    }
}
if(instance_exists(obj_kitchenchecker ) && spot < 2 && obj_kitchenchecker.level == 1 && global.interact == 0) {
    if(spot == 1) {
        myscore++;
        if(myscore > 3) {
            snd_play(snd_bell );
