...
if(global.mnfight == 2 && attacked == 0) {
    ...
    gen.myself= myself;
    if(mycommand >= 0)
        global.msg[0]= "* Temmie is trying to glomp&  you.";
    if(mycommand >= 25)
        global.msg[0]= "* Temmie forgot her other&  attack.";
    if(mycommand >= 50) global.msg[0]= "* Temmie vibrates intensely.";
    if(mycommand >= 75) global.msg[0]= "* Temmie is doing her hairs.";
    if(mycommand >= 83)
        global.msg[0]= "* Temmiy accidentally misspells&  her own name.";
    if(mycommand >= 90) global.msg[0]= "* Smells like Temmie Flakes.";
    if(global.monsterhp[myself] < 3)
        global.msg[0]= "* How\\'d you do that?";
    attacked= 1;
}
if(global.myfight == 2 && whatiheard != -1 && global.heard == 0) {
    if(whatiheard == 0) {
        global.msc= 0;
        global.msg[0]= "* TEMMIE - RATED TEM OUTTA TEM&* Loves to pet cute humans^1.&* But you\\'re allergic!/^";
        OBJ_WRITER.halt= 3;
    ...
    if(whatiheard == 1) {
        ...
        if(haveit == 0) {
            global.msc= 0;
            global.msg[0]= "* Temmie only wants the&  Temmie Flakes./^";
            OBJ_WRITER.halt= 3;
            iii= instance_create(global.idealborder[0], global.idealborder[2], OBJ_WRITER );
            with(iii) halt= 0;
        } else  {
            ...
            global.myfight= 3;
            global.bmenuno= 6;
            global.msg[0]= "   Give the Temmie Flakes?& &   Yas             NO!!!!!!!\\\\C";
            OBJ_WRITER.halt= 3;
    ...
    if(whatiheard == 3) {
        global.msc= 0;
        global.msg[0]= "* You flex at Temmie.../%%";
        flex++;
        if(global.at < 150) global.at+= 7;
        OBJ_WRITER.halt= 3;
        iii= instance_create(global.idealborder[0], global.idealborder[2], OBJ_WRITER );
        with(iii) halt= 0;
    ...
    if(whatiheard == 4) {
        global.flag[146]= 1;
        global.msc= 0;
        global.msg[0]= "* You say hello to Temmie./^";
        OBJ_WRITER.halt= 3;
        iii= instance_create(global.idealborder[0], global.idealborder[2], OBJ_WRITER );
        with(iii) halt= 0;
        mercymod= -10;
    ...
    if(whatiheard == 7) {
        ...
        // OBJ_INSTAWRITER
        with(785) instance_destroy();
        with(mypart1) sprite_index= spr_temsmug ;
        with(mypart1) alarm[9]= 30;
    }
    global.heard= 1;
}
if(whatiheard == 3 && !instance_exists(OBJ_WRITER ) && dunk == 0) {
    blconx= instance_create(x + 95, y - 25, obj_blconsm );
    global.msg[0]= "NO!!!!!&muscles&r.....&NOT CUTE/%%";
    dunk= 1;
    global.msg[1]= "%%%";
    global.typer= 2;
    blconxwd= instance_create(blconx.x + 15, blconx.y + 10, OBJ_NOMSCWRITER );
}
if(dunk == 1 && !instance_exists(OBJ_WRITER )) {
    ...
    blconx.depth= 50;
    global.msg[0]= "Ooh, I&have to&disagree&; )/%%";
    if(global.flag[95] == 1) global.msg[0]= "Temmie^1,&are you&okay?&; )/%%";
    global.msg[1]= "%%%";
    global.typer= 2;
    global.msc= 0;
...
if(dunk == 2 && !instance_exists(OBJ_WRITER )) {
    with(blconx) instance_destroy();
    image_index= 1;
    blconx= instance_create(x + 95, y - 25, obj_blconsm );
...
if(dunk == 3 && !instance_exists(OBJ_WRITER )) {
    with(blconx) instance_destroy();
    mercymod= 222;
    obj_aaron.flex= 2;
}
