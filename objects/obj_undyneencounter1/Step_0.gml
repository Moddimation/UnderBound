if(active == 1) {
    if(instance_exists(obj_mainchara ) && cn == 0 && global.interact == 0 && obj_mainchara.x > x) {
        global.interact= 1;
    ...
    if(cn == 1.2) {
        if(ar.image_alpha > 0.02)
            ar.image_alpha-= 0.1;
        if(__view_get( e__VW.YView, 0 ) > 10) __view_set( e__VW.YView, 0, __view_get( e__VW.YView, 0 ) - (5) );
        else  cn= 2;
    }
    if(cn == 2) {
